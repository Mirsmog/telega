...
async def send_all_order():
    ...
    for order in orders:
        ...
        for user in users:
            ...
            # Проверяем, работает ли пользователь в том же регионе и есть ли у него значение true для этого региона
            if (
                is_region_present(order, user)
                and user["all_regions"][order["regions"]]["subreg"].get(order["preregion"]) == True
            ):
                                    ...
                                    if set1.issubset(set2) and user["cars"][car]["status"] == "search":
                                        if order["type_order"] == "people":
                                            text = (
                                                ...
                                                f"Пассажирские перевозки\n"
                                                f"{order['type_car']} количество машин {order['amount_car']}\n"
                                                f"Дата: {format_date_to_russian(order['date'])}\n {order['regions']}, {order['address']}\n"
...
async def chek_order_amount(user_id):
    ...
    if check["setting_limit"] > 0:
        return "go_send"
    elif check["setting_limit"] == 0 and check["balance"] >= 70:
        return "buy"
    else:
        return (
            f'Бесплатно можно подать {check["main_limit"]} заявки в день. Размещение каждой '
            f"последующей заявки стоит 70р"
        )
